...
def evaluate_model_diff(method, model, new_model, x_valid, y_valid, remove_x, remove_y, val_remove_x, val_remove_y,
                        val_remain_x, val_remain_y, diverged=False, verbose=False, log_dir=None, log_path='models/log',
    ...
    for index, element in enumerate(y_pred):
        if element != y_valid_int[index] and y_pred_org[index] == y_valid_int[index] and y_pred_org[
            index] == targetclass:
            y_pred_wrong_prediction.append(
                {"index": index, "org_prob": y_pred_org_prob[index], "after_prob": y_pred_prob[index],
                 "after_predict": element})
...
def evaluate_unlearning(model_init, model_weights, data,
    ...
    acc_before, acc_after, diverged = evaluate_model_diff(
        'Unlearn_tau=' + str(unlearn_kwargs['tau']) + '_type=' + str(unlearn_type) + '_order=' + str(order),
        model, new_model, x_valid, y_valid, remove_x, remove_y, val_remove_x, val_remove_y, val_remain_x, val_remain_y,
        diverged=diverged, verbose=verbose, log_dir=log_dir, log_path=log_path,
...
def unlearn_update(z_x, z_y, removedData, model, x_val, y_val, validRemovedClass, validRemainClass,
                   removed_y_train_random, unlearn_kwargs,
    ...
    with GradientLoggingContext('unlearn'):
        new_theta, diverged, duration_s = iter_approx_retraining(z_x, z_y, model, x_val, y_val, remove_x, remove_y,
                                                                 ...
                                                                 verbose=verbose, cm_dir=cm_dir, log_dir=log_dir,
        ...
        for step in range(0, steps + 1):
            ...
            if step == 0:
                ...
                for istep in range(1, max_inner_steps + 1):
                    ...
                    new_theta, diverged = approx_retraining(model, remove_x, remove_y, removed_y_train_random,
                                                            order=order,
            ...
            with measure_time() as t:
                ...
                print(
                    f">> iterative approx retraining: step = {step}, train_acc (before/after) = {batch_acc_before} / {batch_acc_after}, "
...
def fine_tuning(model_init, model_weight, data, removed_y_train_random, removedData, validRemovedClass,
                validRemainClass, epochs=1, clean_acc=1.0, repaired_filepath='finetune_model.hdf5', train_kwargs=None,
                log_dir=None, log_path=None, unlearn_kwargs=None, valid_orgc_near_attc=None, valid_attc_set=None):
    clear_session()
    ...
    with measure_time() as t:
        model.fit(remove_x, removed_y_train_random, validation_data=(x_test, y_test), verbose=1, epochs=epochs,
    ...
    acc_before, acc_after, _ = evaluate_model_diff('finetune', model, new_model, x_valid, y_valid, remove_x, remove_y,
                                                   val_remove_x, val_remove_y, val_remain_x, val_remain_y,
...
def train_retrain(model, train, val, test, model_folder, data, removedData, validRemovedClass, validRemainClass,
                  dataset, epochs=150, batch_size=64, data_augmentation=False, log_path='models/log',
                  valid_orgc_near_attc=None, valid_attc_set=None):
    ...
    (x_val, y_val) = val
    (x_test, y_test) = test
    hist = model.fit(x_train, y_train, batch_size=batch_size, epochs=100, validation_data=test, verbose=1,
    ...
    (val_remove_x, val_remove_y) = validRemovedClass
    (val_remain_x, val_remain_y) = validRemainClass
    acc_before, acc_after, _ = evaluate_model_diff('retrain', best_model, best_model, x_valid, y_valid, remove_x,
                                                   remove_y, val_remove_x, val_remove_y, val_remain_x, val_remain_y,
                                                   log_path=log_path, valid_orgc_near_attc=valid_orgc_near_attc,
                                                   valid_attc_set=valid_attc_set)
