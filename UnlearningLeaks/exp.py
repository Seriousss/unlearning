...
class ExpMemInfScratch(ExpMemInf):
    ...
    def _obtain_posterior(self, num_sample, num_shard, model_type, save_path):
        self.record_split.generate_sample(model_type)
        posterior_df_list = []  # 创建一个列表来存储每个DataFrame
        ...
        for sample_index in range(num_sample):
            ...
            for unlearning_set_index, unlearning_indices in unlearning_set.items():
                ...
                for index in range(post_before_pos.shape[0]):
                    df.loc[len(df)] = [post_before_pos[index].reshape([1, -1]), post_after_pos[index].reshape([1, -1]),
                                       1]
                ...
                for index in range(post_before_neg.shape[0]):
                    df.loc[len(df)] = [post_before_neg[index].reshape([1, -1]), post_after_neg[index].reshape([1, -1]),
                                       0]

                posterior_df_list.append(df)  # 将df添加到列表中

        posterior_df = pd.concat(posterior_df_list, ignore_index=True)  # 使用pd.concat合并所有的DataFrame
        return posterior_df
